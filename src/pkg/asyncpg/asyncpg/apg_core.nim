...
iterator instantRows*(pgres: PPGresult, columns: var DbColumns,
                      args: varargs[string, `$`]): InstantRow
                      {.tags: [ReadDbEffect].} =
  ## same as fastRows but returns a handle that can be used to get column text
  ## on demand using []. Returned handle is valid only within iterator body.
  setColumnInfo(columns, pgres, pqnfields(pgres))
  for i in 0'i32..pqNtuples(pgres)-1:
    yield InstantRow(res: pgres, line: i)
  pqClear(pgres)
